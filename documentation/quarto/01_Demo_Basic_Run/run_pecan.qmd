...
# - NEE (Net Ecosystem Exchange), TotalResp (Total Respiration)
# - AutoResp (Autotrophic Respiration), HeteroResp (Heterotrophic Respiration)
# - SoilResp (Soil Respiration)
# Carbon pools:
# - AbvGrndWood (Above Ground Wood), TotLivBiom (Total Live Biomass)
# - TotSoilCarb (Total Soil Carbon), leaf_carbon_content
# - litter_carbon_content, fine_root_carbon_content, coarse_root_carbon_content
# - GWBI (Gross Woody Biomass Increment), AGB (Above Ground Biomass)
# Water variables:
# - Qle (Latent Heat Flux), Transp (Transpiration)
# - SoilMoist (Soil Moisture), SoilMoistFrac (Soil Moisture Fraction)
# - SWE (Snow Water Equivalent), litter_mass_content_of_water
# Other:
# - LAI (Leaf Area Index)

# Example 1: Carbon fluxes
# Plot Gross Primary Productivity (GPP) and Net Primary Productivity (NPP)
# These represent the carbon uptake by photosynthesis
...
# Add NPP line to compare with GPP
lines(model_output$posix, model_output$NPP, 
      col = 'blue')

# Add legend to distinguish between the two variables
legend('topright', 
       legend = c('GPP', 'NPP'),
       col = c('green', 'blue'),
       lty = 1)
