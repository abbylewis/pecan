...
PEPRMT_CH4_FINAL <- function(theta,
  ...
  for(i in 1:length(q)){
    ...
    #Time Invariant
    WTD_m_adj <- d$WTD_cm/100 + bottom_bound_m #convert to m and adjust because we are modeling from 1m below the sediment to the atmosphere
    WTD_m_adj[WTD_m_adj < 0] <- 0.000001 #make sure WTD_m_adj is never negative or 0 (WTD < bottom bound)
    R <- 8.314 #J K mol-1
    ...
    #k_hydro <- ifelse(WTD_m_adj > bottom_bound_m, k_hydro_max, 
    ...
    Hydro_flux_tot <- vector('numeric', length(d$DOY))
    ...
    for(t in 1:length(d$DOY)) {
      ...
      if (t == 1) { 
        ...
        CH4water_g_prev <- CH4water_store[t-1] * WTD_m_adj[t-1]
        #If water level decreased, assume some methane left with it
        if(WTD_m_adj[t-1] > WTD_m_adj[t]) { 
          CH4_exported <- CH4water_g_prev *
            ((WTD_m_adj[t-1] - WTD_m_adj[t]) / WTD_m_adj[t-1]) * #fractional water loss
            fract_outflow #fraction of methane exported
        } else {
          CH4_exported <- 0
        }
        #account for any changes in concentration of CH4 due to any change in WTD_m_adj height
        CH4water_init <- CH4water_g_prev - CH4_exported # gC
      } 
      
      #Total methane prior to oxidation (gC CH4 m-3)
      CH4water_pre_oxi <- (CH4prod[t] + CH4water_init) / WTD_m_adj[t]
      
      if(WTD_m_adj[t] > bottom_bound_m){
        CH4_oxi <- 0 #No oxidation when water is above the soil
      } else {
        Vmax_CH4oxi[t] <- Vmax_CH4oxi[t] + Vmax_CH4oxi[t] * (bottom_bound_m - WTD_m_adj[t])
      ...
      if(WTD_m_adj[t] > bottom_bound_m){
        Hydro_flux_tot[t] <- k_hydro_max * CH4water[t] #gC CH4 m-2 day-1  Hydrodynamic flux 
        Hydro_flux[t] <- k_hydro_max * CH4water[t]  #gC CH4 m-2 day-1  Hydrodynamic flux 
      } else {
        Hydro_flux_tot[t] <- k_hydro_max * CH4water[t] * WTD_m_adj[t]/bottom_bound_m # less diffusion
        Hydro_flux[t] <- Hydro_flux_tot[t] - 
          Hydro_flux_tot[t] * V_oxi_hydro * WTD_m_adj[t]/bottom_bound_m # more oxidation
      }
      ...
      CH4water_store[t] <- CH4water[t] - Hydro_flux_tot[t] - Plant_flux[t]  #gC CH4 m-3 stored in the system
