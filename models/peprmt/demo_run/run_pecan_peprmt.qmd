...
```         
remotes::install_github("pecanproject/pecan", subdir = "base/all", ref = "develop", force = T)
...
model_output |>
  mutate(site_id = sub("ENS-\\d+-", "", run_id)) |>
  select("site_id", "run_id", "posix", "slow_soil_pool_carbon_content",
         "fast_soil_pool_carbon_content") |>
  pivot_longer(cols = c("slow_soil_pool_carbon_content",
                        "fast_soil_pool_carbon_content"), 
         names_to = "flux") |>
  ggplot() +
  aes(x = posix, y = value, color = flux, group = run_id) +
  facet_grid(flux~site_id, scales = "free_y") +
  geom_line() +
  xlab("Date") +
  ylab("Carbon Flux (kg C m-2 s-1)") +
  ggtitle("Carbon Fluxes Over Time") +
