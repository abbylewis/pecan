...
MCMC_block_function <- function(block) {
  ...
  if (is.list(dat)) {
    # dat: list(chain1 = matrix, chain2 = matrix, ...)
    # get a (niter_total x nparam) matrix
    ...
    if (fresh.run) {
      for (i in seq_along(block.list)) {
        ...
        if (block.list[[i]]$constant$q.type == 3) {
          ...
          if (!is.null(aqq.Init) && !is.null(bqq.Init) &&
              length(aqq.Init) > 0 && length(bqq.Init) > 0 &&
              !all(is.na(aqq.Init)) && !all(is.na(bqq.Init))) {
...
block.2.vector <- function (block.list, X, H, adjustment) {
  ...
  site.ids <- block.list[[1]]$site.ids
  # In the case of single Site + multiple variables
  # Site has only one value. Copy it to each column
  if (!is.null(site.ids) && length(site.ids) == 1L && ncol(X) > 1L) {
    site.ids <- rep(site.ids, ncol(X))
  }
  mu.f <- mu.a <- rep(NA_real_, length(site.ids))
