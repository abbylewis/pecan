...
* Datasets
  ...
  *  `ca_n_application_rate` dataset with recommended N application rates (kg N/ha) for 33 California crops from CDFA-FREP and UC ANR sources.
  *  `ca_compost_amendment` dataset with C:N ratios, carbon, nitrogen, and PAN (kg/ha) for 32 organic amendment materials.
* Functions
  *  `get_ca_n_rate()` for looking up crop-specific N application rates by name (exact match first, partial match suggestions on miss).
  *  `get_ca_compost_amendment()` for looking up organic amendment properties by material name.
